#[doc = "Register `TFTPIXEL0` reader"]
pub struct R(crate::R<TFTPIXEL0_SPEC>);
impl core::ops::Deref for R {
    type Target = crate::R<TFTPIXEL0_SPEC>;
    #[inline(always)]
    fn deref(&self) -> &Self::Target {
        &self.0
    }
}
impl From<crate::R<TFTPIXEL0_SPEC>> for R {
    #[inline(always)]
    fn from(reader: crate::R<TFTPIXEL0_SPEC>) -> Self {
        R(reader)
    }
}
#[doc = "Register `TFTPIXEL0` writer"]
pub struct W(crate::W<TFTPIXEL0_SPEC>);
impl core::ops::Deref for W {
    type Target = crate::W<TFTPIXEL0_SPEC>;
    #[inline(always)]
    fn deref(&self) -> &Self::Target {
        &self.0
    }
}
impl core::ops::DerefMut for W {
    #[inline(always)]
    fn deref_mut(&mut self) -> &mut Self::Target {
        &mut self.0
    }
}
impl From<crate::W<TFTPIXEL0_SPEC>> for W {
    #[inline(always)]
    fn from(writer: crate::W<TFTPIXEL0_SPEC>) -> Self {
        W(writer)
    }
}
#[doc = "Field `DATA` reader - RGB Data"]
pub type DATA_R = crate::FieldReader<u32, u32>;
#[doc = "Field `DATA` writer - RGB Data"]
pub type DATA_W<'a> = crate::FieldWriter<'a, u32, TFTPIXEL0_SPEC, u32, u32, 24, 0>;
impl R {
    #[doc = "Bits 0:23 - RGB Data"]
    #[inline(always)]
    pub fn data(&self) -> DATA_R {
        DATA_R::new((self.bits & 0x00ff_ffff) as u32)
    }
}
impl W {
    #[doc = "Bits 0:23 - RGB Data"]
    #[inline(always)]
    pub fn data(&mut self) -> DATA_W {
        DATA_W::new(self)
    }
    #[doc = "Writes raw bits to the register."]
    #[inline(always)]
    pub unsafe fn bits(&mut self, bits: u32) -> &mut Self {
        self.0.bits(bits);
        self
    }
}
#[doc = "TFT Pixel 0 Register\n\nThis register you can [`read`](crate::generic::Reg::read), [`write_with_zero`](crate::generic::Reg::write_with_zero), [`reset`](crate::generic::Reg::reset), [`write`](crate::generic::Reg::write), [`modify`](crate::generic::Reg::modify). See [API](https://docs.rs/svd2rust/#read--modify--write-api).\n\nFor information about available fields see [tftpixel0](index.html) module"]
pub struct TFTPIXEL0_SPEC;
impl crate::RegisterSpec for TFTPIXEL0_SPEC {
    type Ux = u32;
}
#[doc = "`read()` method returns [tftpixel0::R](R) reader structure"]
impl crate::Readable for TFTPIXEL0_SPEC {
    type Reader = R;
}
#[doc = "`write(|w| ..)` method takes [tftpixel0::W](W) writer structure"]
impl crate::Writable for TFTPIXEL0_SPEC {
    type Writer = W;
}
#[doc = "`reset()` method sets TFTPIXEL0 to value 0"]
impl crate::Resettable for TFTPIXEL0_SPEC {
    #[inline(always)]
    fn reset_value() -> Self::Ux {
        0
    }
}
